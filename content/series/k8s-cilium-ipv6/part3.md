...
In this part of the series we explored key network and security architecture considerations to understand and plan for *before* deploying Kubernetes, while also touching upon some of Cilium's features which can be leveraged to meet those architecture requirements. In the next part of this series I will demonstrate (with example) how Cilium's multi-pool IPAM with native-routing can be used to achieve all the points I laid out in [Kubernetes IPv6 Design Philosophy](#kubernetes-ipv6-design-philosophy)
